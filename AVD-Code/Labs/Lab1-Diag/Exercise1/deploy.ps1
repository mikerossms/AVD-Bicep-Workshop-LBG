...
if (-not $username) {
    Write-Error "Please provide your username - e.g. 'deploy.ps1 -username jbloggs'"
    exit 1
}
...
Write-Host "Deploying diagnostic.bicep to Resource Group: $rg" -ForegroundColor Green
New-AzResourceGroupDeployment -Name "Deploy-Diagnostics" -ResourceGroupName $rg -TemplateFile "$PSScriptRoot/Bicep/Diagnostics/diagnostics.bicep" -Verbose 
